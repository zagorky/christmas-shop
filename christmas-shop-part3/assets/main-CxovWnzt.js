import { c as createElem, T as TABLET_SCREEN_WIDTH, i as initBurger, a as initGiftsGetData } from "./helper-CrgfauEd.js";
const timerData = {
...
function createLastTime(elem, ms) {
  switch (elem) {
    ...
    case "hours":
      return Math.floor(ms / (1e3 * 60 * 60) % 24);
    case "mins":
      return Math.floor(ms / (1e3 * 60) % 60);
    case "secs":
      return Math.floor(ms / 1e3 % 60);
    default:
      return "0";
  }
}
...
function moveSlider(direction) {
  ...
  let directionValue = direction === "left" ? +offset.toFixed(0) : -offset.toFixed(0);
